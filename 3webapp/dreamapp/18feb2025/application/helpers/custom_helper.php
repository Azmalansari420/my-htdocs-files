...
function user_idname($id)
{
  $ci = & get_instance();
  $size = $ci->crud->selectDataByMultipleWhere('users',array('user_id'=>$id,));
  if(!empty($size))
  {
    $name = $size[0]->name;
  }
...
{
    $ci = & get_instance();
    $current_lang = $ci->session->userdata('current_lang') ?: 'english';
    
    $return_text = $text;
    $path = APPPATH . "language/$current_lang/messages.php";
...
function add_type_history($value)
{
  $string = '';
  if($value==1)
  {
  ...
  {
    $string = 'Game Win Amount';
  }
  else if($value==4)
  {
    $string = 'Withdraw Request';
  }
  else if($value==5)
  {
    $string = 'Refund';
  ...
  {
    $string = 'Referal Amount';
  }
  else if($value==7)
  {
    $string = 'Point Add By Admin';
  }
  else if($value==8)
  {
    $string = 'Point Deduct By Admin';
  }
  else if($value==9)
  {
    $string = 'Point Transfer';
  }
  else if($value==10)
  {
    $string = 'Point Recieved';
  }
  return $string;
}
...
function credit_debit($value)
{
  $string = '';
  if($value==1)
  {
    $string = '<span class="btn btn-success btn-sm"> Add (Credit)</span>';
  }
  else if($value==2)
  {
    $string = '<span class="btn btn-danger btn-sm"> Deduct (Debit)</span>';
  }
  return $string;
}
