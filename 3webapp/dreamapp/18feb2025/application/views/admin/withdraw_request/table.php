<table class="table table-striped table-td-valign-middle table-bordered bg-white">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>
          Username:- <?=username($data->user_id) ?><br>
          User ID:- <?=get_user_id($data->user_id) ?><br>
      </td>
      <td><?=$data->requestid ?></td>
      <td><?=$data->amount ?></td>
      <!-- <td>
        Bank Name:- <?=$data->bank_name ?><br>
        Account Holder Name:- <?=$data->ac_name ?><br>
        Account Number:- <?=$data->ac_no ?><br>
        IFSC Code:- <?=$data->ifsc_code ?><br>
      </td> -->
      <!-- <td><?=$data->upi_no ?></td> -->
      <td><?=add_point_status($data->status) ?></td>
      <td>
        <form action="<?=base_url('admin_con/withdraw_request/withdraw_request_status') ?>" method="post">
