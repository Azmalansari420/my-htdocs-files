    ...
    <tr>
      ...
      <td>
         <a href="<?=base_url($upload_path.$data->profile_image) ?>" target="_blank">
            <img src="<?=base_url($upload_path.$data->profile_image) ?>" width="50px">
         </a>
      </td>

      <td><?=$data->name ?></td>

      <td>DA<?=($data->user_id) ?></td>
      ...
      <td class="btn-col text-nowrap" width="1%">
        <a href="<?php echo $view_url.'?user_id='.$data->id; ?>" class="btn btn-info btn-xs m-r-2">View History</a>
        <!-- <a href="<?php echo base_url('admin_con/users/playview?user_id='.$data->id) ?>" class="btn btn-success btn-xs m-r-2">Play History</a> -->
