 ...
 function place_bet() 
{
    ...
    inputs.forEach(input => {
        ...
        const value = input.value.trim(); // Trim to remove spaces
        ...
        if (isNaN(amount) || amount < 5) {
            toaster(`Minimum bet amount for ${number} is 5. (e.g., 5, 10, 15...)`, 'error');
            isValid = false;
            return;
        }

        if (amount % 5 !== 0) {
            toaster(`Bet amount for ${number} should be in multiples of 5 (e.g., 5, 10, 15...)`, 'error');
            isValid = false;
            return;
        }
    });
    ...
    if (!isValid) {
        return; // Stop the function if any input is invalid
    }

    // If all amounts are valid, proceed with form submission
    inputs.forEach(input => {
        const number = input.name;
