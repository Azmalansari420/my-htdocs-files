...
if(!empty($requestid))
{
    $getwhatsapp = $this->db->get_where('add_point_request',array('requestid'=>$requestid))->result_object();

    $username = $getwhatsapp[0]->user_id;
    $amount = $getwhatsapp[0]->amount;
    $utr_no = $getwhatsapp[0]->utr_no;
   

    $message = "Username:-`.$username.`<br> Amount:-`.$amount.`<br> UTR NO:-`.$utr_no.`<br>";
}
  ...
  $(document).on("click", ".submit-btn",(function(e) {
    ...
    function add_utrno()
    {
        ...
        $.ajax(settings).done(function (response) 
        {
          ...
          {
            var whatsapp_url = "https://api.whatsapp.com/send?phone=91<?=$site_setting[0]->mobile ?>&text=UTR No: " + utr_no + "%0ARequest ID: " + requestid;
