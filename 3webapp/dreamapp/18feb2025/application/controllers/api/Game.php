...
/*game bet ---jodi--*/
    ...
    public function jodi_place_bet()
    {
        ...
        // Get Game Timings
        $game = $this->db->get_where('game', array('id' => $game_id, 'status' => 1))->row();
        if (!$game) {
            echo json_encode(["status" => 400, "message" => "Invalid game or game not available.", "data" => []]);
            return;
        }

        $open_time = date('H:i:s', strtotime($game->open_time));
        $close_time = date('H:i:s', strtotime($game->close_time));

        if ($close_time < $open_time) 
        {
            if (!($time >= $open_time || $time <= $close_time)) 
            {
                echo json_encode(["status" => 400, "message" => "Betting is closed for this game.", "data" => []]);
                return;
            }
        } 
        else 
