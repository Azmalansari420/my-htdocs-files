...
class Users extends CI_Controller
{
	...
	protected $arr_values = array(
						   	...
						   	'pagination_url'=>'admin_con/users/get_table_data',
   ...
   //--check user login or not
	...
	function playview()
	{
		...
		$data['back_url'] = base_url('admin_con/'.$this->arr_values['back_url'].'/listing');
		// $data['EDITDATA'] = $this->crud->fetchdatabyid($args[0],$this->arr_values['table_name']);
		$this->load->view($this->arr_values['load_path'].'play',$data);
	}
...
//---------------------status
	...
	public function export_to_excel()
	{
	    ...
	    $sheet->setCellValue('F1', 'Wallet');

	    // Fetch data from database
	    $this->db->select('name, id, state_id, mobile, password, wallet');
	    $this->db->order_by('id desc');
	    $game_bets = $this->db->get('users')->result_array();

	    // Populate data into Excel
	    $row = 2;
	    foreach ($game_bets as $bet) {
	        $sheet->setCellValue('A' . $row, $bet['name']);
	        $sheet->setCellValue('B' . $row, get_user_id($bet['id']));
	        $sheet->setCellValue('C' . $row, statename($bet['state_id']));
	        $sheet->setCellValue('D' . $row, $bet['mobile']);
	        $sheet->setCellValue('E' . $row, $bet['password']);
	        $sheet->setCellValue('F' . $row, $bet['wallet']);
	        $row++;
	    }
