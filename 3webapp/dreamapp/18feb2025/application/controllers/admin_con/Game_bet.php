...
//---------------------status
	...
	public function export_to_excel()
	{
	    ...
	    require_once APPPATH . 'libraries/PHPExcel-develop/Classes/PHPExcel.php';
	    ...
	    $sheet->setCellValue('D1', 'Game Type Name');
	    $sheet->setCellValue('E1', 'Game ID');
	    $sheet->setCellValue('F1', 'Date');
	    $sheet->setCellValue('G1', 'Time');

	    // Fetch data from database
	    $this->db->select('number, amount, game_type, game_type_name, game_id, date, time, user_id');
	    $this->db->where('game_id', $game_id);
	    $this->db->where('date', $select_date);
	    $game_bets = $this->db->get('game_bet')->result_array();

	    // Populate data into Excel
	    $row = 2;
	    foreach ($game_bets as $bet) {
	        $sheet->setCellValue('A' . $row, $bet['number']);
	        $sheet->setCellValue('B' . $row, $bet['amount']);
	        $sheet->setCellValue('C' . $row, get_user_id($bet['user_id']));
	        $sheet->setCellValue('D' . $row, gametype($bet['game_type']));
	        $sheet->setCellValue('E' . $row, gamename($bet['game_id']));
	        $sheet->setCellValue('F' . $row, $bet['date']);
	        $sheet->setCellValue('G' . $row, $bet['time']);
	        $row++;
	    }

	    // Set headers for download
	    header('Content-Type: application/vnd.ms-excel');
	    header('Content-Disposition: attachment;filename="game_bets_' . $select_date . '.xls"');
