   ...
   //--check user login or not
	...
	function add()
	{
		...
		if(isset($_POST['submit']))
		{
			...
			$gameamt = $this->db->get_where('win_amount',array('id'=>1,))->result_object()[0];

			$jodi_amt = $gameamt->jodi_amt;
			$haruf_ander_amt = $gameamt->haruf_ander_amt;
			$haruf_baher_amt = $gameamt->haruf_baher_amt;


			/*game jodi*/
			$game_bet  = $this->db->get_where('game_bet',array('game_id'=>$game_id,'date'=>$create_on,'number'=>$number,'game_type'=>1))->result_object();
			...
			if(!empty($game_bet))
			{
				...
				{
					$amount_to_distribute = $value->amount * $jodi_amt; // ₹1 = ₹95
			...
			if(!empty($game_haruf_ander_bet))
			{
				...
				if (!empty($game_haruf_ander_bet)) 
				{
				    ...
				    {
				        $current_first_digit = ($value->number); 
				        $current_modified_first_digit = $current_first_digit;
				        
				        // print_r($current_modified_first_digit."<br>");
				        ...
				        if ($current_modified_first_digit == $modified_first_digit) 
				        {
				            ...
				            $amount_to_distribute = $value->amount * $haruf_ander_amt;
				            ...
				            $type = "credit";
				            $amount = $amount_to_distribute;
				            $message = 'Game Win in ' . gamename($game_id) . ' Ander (' . harup_ander($current_modified_first_digit) . ')';
				            $this->crud->wallet_credit_debit($request_id, $user_id, $type, $add_type, $amount, $message);
				        }
				    }
				}
				...
				if (!empty($game_haruf_bahar_bet)) 
				{
				    foreach ($game_haruf_bahar_bet as $key => $value) 
				    {
				        ...
				        if ($current_modified_last_digit == $modified_last_digit) 
				        {
				            ...
				            $amount_to_distribute = $value->amount * $haruf_baher_amt;
				            ...
				            $type = "credit";
				            $amount = $amount_to_distribute;
				            $message = 'Game Win in ' . gamename($game_id) . ' Bahar (' . harup_baher($current_modified_last_digit) . ')';
				            $this->crud->wallet_credit_debit($request_id, $user_id, $type, $add_type, $amount, $message);
				        }
				    }
				}
