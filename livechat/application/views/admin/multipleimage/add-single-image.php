<div class="row card-body" style="border: 1px solid;margin-bottom: 5px;padding: 5px;">
    ...
    <div class="col-4 form-group">
       ...
       if(!empty($value->single_image))
       	{ ?>
       		<input type="hidden" name="single_image_old[]" value="<?=$value->single_image ?>">
       		<img src="<?=base_url($upload_path.$value->single_image) ?>" width="50px" style="margin-top: 5px;">
   <?php } ?>
    </div>
    
    <button type="button" class="btn btn-sm btn-red remove-btn">Remove</button>
</div>